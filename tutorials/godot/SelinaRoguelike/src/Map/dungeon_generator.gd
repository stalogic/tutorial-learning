...
@export var max_monsters_per_room: int = 3
...
const entity_types = {
	"orc": preload("res://src/Assets/Definnitions/Entities/Actors/entity_definition_orc.tres"),
	"troll": preload("res://src/Assets/Definnitions/Entities/Actors/entity_definition_troll.tres"),
	"health_potion": preload("res://src/Assets/Definnitions/Entities/Items/health_potion_definition.tres"),
}
...
func _crave_room(dungeon: MapData, room: Rect2i) -> void:
...
func _place_entities(dungeon: MapData, room: Rect2i) -> void:
	var num_of_monsters: int = _rng.randi_range(0, max_monsters_per_room)
	var num_of_items: int = _rng.randi_range(0, max_items_per_room)
	
	for _i in num_of_monsters:
		var x: int = _rng.randi_range(room.position.x+1, room.end.x-1)
		var y: int = _rng.randi_range(room.position.y+1, room.end.y-1)
		var new_enitiy_position := Vector2i(x, y)
		
		var can_place: bool = true
		for entity in dungeon.entities:
			if entity.grid_position == new_enitiy_position:
				can_place = false
				break
		...
		if can_place:
			...
			if _rng.randf() < 0.8:
				new_entity = Entity.new(dungeon, new_enitiy_position, entity_types.orc)
			else:
				new_entity = Entity.new(dungeon, new_enitiy_position, entity_types.troll)
			dungeon.entities.append(new_entity)
			
	for _i  in num_of_items:
		var x: int = _rng.randi_range(room.position.x+1, room.end.x-1)
		var y: int = _rng.randi_range(room.position.y+1, room.end.y-1)
