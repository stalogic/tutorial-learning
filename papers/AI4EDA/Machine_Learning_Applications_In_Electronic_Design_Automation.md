...
为应对时间分析的不确定性，传统VLSI设计采用悲观估计或迭代回溯策略，但这些方法可能导致过度设计或增加设计周期。串扰是影响信号完整性和时间收敛的主要问题，通常在详细布线后才能准确分析，此时修正空间有限。
...
传统设计使用两种时序模型：准确但缓慢的签核模型和快速但不精确的综合模型。ML辅助方法在提供快速且精确的时序和串扰预测方面显示潜力。
...
#### 3.4.5 Machine Learning Engines
